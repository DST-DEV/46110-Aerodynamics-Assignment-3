...
Clw = pi*AR*An(1,:); % Lift coefficient of wing
Cdi = pi*AR*sum(n.*(An.^2), 1); % Induced drag coefficient of wing
...
Pmin0 = min(P);
Pmin = min(P); % Calculating the minimum power
while Pmin > Pmin0*0.9
    density_w = density_w*0.7; % [kg/m3] Wing density
    for ii = 1:length(b)
        % ASSUMPTIONS
        c = b(ii)/AR;
        S = b(ii)*c; % [m2] Wing area
...
[~,ind] = min(P);
b_opt = b(ind); % Optimum blade length


%% PLOTTING THE RESULTS
figure(1)
ax = gca;
plot(b,P,'k', LineWidth=1.5)
set(ax,'FontSize',16);
...
fprintf('RESULTS T6.2\n')
fprintf('------------')
fprintf('\nDensity: %.2f kg/m^3',density_w)
fprintf('\nChord: %.2f m',c)
fprintf('\nBlade length: %.2f m',b_opt)
